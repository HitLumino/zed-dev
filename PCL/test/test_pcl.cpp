...

    {
        ...
        if(i==0)
        {
            if(zed.grab(init_grab)==SUCCESS);
            {
                ...
                cv::cvtColor(slMat2cvMat(image_depth),frame_depth,CV_BGRA2GRAY);
        ...
        else
        {
           ...
           // cloud_last= cloud_cur;
            ...
            if(zed.grab(init_grab)==SUCCESS);
            {
                ...
                cout<<"不加显示耗时："<<time1.elapsed()<<endl;
                cout<<result.R<<endl;
               ...
               T(2,3) = result.tvec(2);
               boost::timer time3;
               cloud_last = joinPointCloud( cloud_last,cloud_cur, T);
               //Eigen::Isometry3d T = cvMat2Eigen( result.rvec, result.tvec );
               cout<<"合并耗时："<<time3.elapsed()<<endl;

//                 cout<<"Final result saved."<<endl;
                  boost::timer time4;
                  viewer.showCloud( cloud_last );
                  cout<<"显示耗时："<<time4.elapsed()<<endl;
        ...
        cout<<i++<<endl;
        cout<<"fps:  "<<zed.getCurrentFPS()<<endl;
